...
export const PlayButtonLibrary = ({playlist}) => {
  const {
    activatePlayOn, handleIndex, changeCurrentList, musicState, disablePlayOn
      ...
      >
        {!playOn && (
          <>
            ...
            <p onClick={handleIndexPlay} className="ml-2 sm:hidden">Play</p>
          </>
        )}
        {playOn && (
          <>
            <FaPause onClick={handleIndexPause} className="w-5 sm:w-20" />
            <p onClick={handleIndexPause} className="ml-2 sm:hidden">Pause</p>
          </>
        )}
      </button>
    </div>
